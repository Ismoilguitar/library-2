	...
	>
		<thead>
			<tr>
				...
				<th class="max-md:row-start-9 max-md:col-start-1">Нарх</th>
				<th class="max-md:row-start-11 max-md:col-start-1">Санаи бақайдгирӣ</th>
				<th class="max-md:row-start-13 max-md:col-start-1">Муаллиф</th>
				<th class="max-md:row-start-15 max-md:col-start-1">Миқдор</th>
			</tr>
		</thead>
		<tbody>
			<tr v-for="(data, index) in book" :key="index">
				...
				<td :class="{ 'max-md:row-start-[16] max-md:col-start-': index + 1 }">
					{{ data.kol }}
				</td>
			</tr>
		</tbody>
...
const getBook = async () => {
	try {
		const response: ApiResponse = await $fetch('/api/book', {
			method: 'get',
		})
